...
<body class="hold-transition sidebar-mini layout-fixed">
							...
							<tbody>
								<c:forEach items="${users}" var="item">
									<tr>
										...
										<td>${item.email}</td>
										<td>
											<a href="<c:url value='/admin/user?action=editUser&username=${item.username}'/>"  type="button"
												class="btn btn-block btn-success btn-sm">Edit</a>
												
											<button type="button" onclick="deleteUser('${item.username}')"
	...
	function deleteUser(username){
		$.ajax({
			url: '/Java4Final/admin/user?action=deleteUser&username=' + username
		}).then(function(data) {
